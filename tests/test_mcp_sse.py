"""End-to-end test: start MCP SSE server, then call tools/list.

This script spins up the MCP server in SSE mode as a subprocess, waits
for it to become ready, connects via the MCP Python SDK's SSE client,
calls initialize + tools/list, validates the response, then tears down.

Usage:
    uv run python tests/test_mcp_sse.py          # standalone
    make test-mcp                                 # via Makefile

Why SSE integration test?
    ...
    layer works: the server boots, accepts SSE connections, and responds
    to the MCP protocol handshake + tools/list correctly.
"""
...
def start_mcp_server() -> subprocess.Popen:
    """Start the MCP server in SSE mode as a background subprocess.
    ...
    """
    env = {**os.environ, "MCP_TRANSPORT": "sse", "PORT": str(PORT)}
...
async def run_tools_list() -> list[dict]:
    """Connect to the MCP SSE server and call tools/list.
...
def main() -> int:
    """Run the MCP SSE integration test.
